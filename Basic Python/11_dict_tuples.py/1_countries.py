""" 1. We have following information on countries and their population (population is in crores),

    |Country|Population|
    |-------|----------|
    |China|143|
    |India|136|
    |USA|32|
    |Pakistan|21|

    1. Using above create a dictionary of countries and its population
    2. Write a program that asks user for three type of inputs,
    
        1. print: if user enter print then it should print all countries with their population in this format,
            ```
            china==>143
            india==>136
            usa==>32
            pakistan==>21
            ```
        1. add: if user input add then it should further ask for a country name to add. If country already exist in our dataset then it should print that it exist and do nothing. If it doesn't then it asks for population and add that new country/population in our dictionary and print it
        2. remove: when user inputs remove it should ask for a country to remove. If country exist in our dictionary then remove it and print new dictionary using format shown above in (a). Else print that country doesn't exist!
        3. query: on this again ask user for which country he or she wants to query. When user inputs that country it will print population of that country.
 """

countries = {
    "china" : 143,
    "india":136,
    "usa":32,
    "pakistan":21,
}

def print_all():
     print("All Country and it's population available in our database")
     for country,population in countries.items():
        print(f"{country}==>{population}")

def add_country():
    country = input("Enter Country name to add : ").lower()
    if country in countries:
        print("country already exist") 
    else:
        population = int(input("Enter it's population : ")) 
        countries.update({country:population})
        print(f"{country} added to our database")
        print_all()

def remove_country():
    country = input("Enter Country name to remove : ").lower()
    if country not in countries:
        print("country doesn't exist in our database") 
    else:
        countries.pop(country)
        print_all()

def query_country():
    country = input("Enter Country name to query : ").lower()
    if country not in countries:
        print("country doesn't exist in our database") 
    else:
        print(f"population of {country} is {countries[country]}")

user_input = input("What do you want (print / add / remove / query) : ")

if user_input == "print":
    print_all()
elif user_input =="add":
    add_country()
elif user_input =="remove":
    remove_country()
elif user_input =="query":
    query_country()
else:
    print("WRONG INPUT!!!")
